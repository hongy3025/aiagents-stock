...
│   │
│   ├── 📁 data/ - 数据获取层
│   │   ├── stock_data.py               # 核心股票数据获取器，集成多数据源(akshare/yfinance/tushare)，计算技术指标
│   │   ├── data_source_manager.py      # 数据源冗余管理器，实现TDX→AKShare→Tushare→YFinance的自动降级切换
│   │   ├── longhubang_data.py          # 龙虎榜数据获取，对接StockAPI获取游资席位、买卖金额、上榜原因
│   │   ├── sector_strategy_data.py     # 智策板块数据采集，获取行业/概念板块行情、北向资金、财经快讯
│   │   ├── smart_monitor_data.py       # AI盯盘数据获取，获取实时行情、分时数据、技术指标用于AI决策
│   │   ├── smart_monitor_tdx_data.py   # TDX本地数据源接口，连接通达信API获取低延迟行情数据
│   │   ├── main_force_selector.py      # 主力选股数据获取，通过pywencai获取主力资金净流入TOP100股票
│   │   ├── fund_flow_akshare.py        # 基于AKShare的资金流向数据获取
│   │   ├── market_sentiment_data.py    # 市场情绪数据采集，获取涨跌家数、涨停跌停统计
│   │   ├── quarterly_report_data.py    # 季报财务数据获取
│   │   ├── risk_data_fetcher.py        # 风险数据采集器，获取限售解禁、大股东减持、重要事件
│   │   └── qstock_news_data.py         # 新闻数据获取
│   │
│   ├── 📁 ai/ - AI智能体系统
│   │   ├── deepseek_client.py          # DeepSeek API客户端封装，支持chat和reasoner双模型
│   │   ├── ai_agents.py                # 核心多智能体协调器，包含技术/基本面/资金/风险4位分析师
│   │   ├── longhubang_agents.py        # 智瞰龙虎5位AI分析师：游资行为、个股潜力、题材追踪、风险控制、首席策略师
...
│   │
│   ├── 📁 db/ - 数据库层(SQLite)
│   │   ├── database.py                 # 核心分析记录数据库，存储股票分析结果、AI报告、最终决策
│   │   ├── monitor_db.py               # 实时监测数据库，管理监测股票列表、价格历史、通知记录
│   │   ├── longhubang_db.py            # 智瞰龙虎数据库，存储龙虎榜原始数据、AI评分、分析报告
│   │   ├── sector_strategy_db.py       # 智策板块数据库，保存板块分析结果、预测历史
│   │   ├── smart_monitor_db.py         # AI盯盘数据库，记录盯盘任务、AI决策历史、交易信号
│   │   ├── portfolio_db.py             # 持仓管理数据库，存储用户持仓信息和分析历史
│   │   └── main_force_batch_db.py      # 主力选股批量分析数据库
│   │
│   ├── 📁 engines/ - 业务引擎层
│   │   ├── longhubang_engine.py        # 智瞰龙虎分析引擎，协调数据获取→AI分析→评分排名→报告生成
│   │   ├── sector_strategy_engine.py   # 智策板块分析引擎，驱动4位AI智能体并行分析
│   │   ├── smart_monitor_engine.py     # AI盯盘核心引擎，实现实时监控→AI决策→交易执行→通知推送
│   │   └── main_force_analysis.py      # 主力选股AI分析模块，对筛选出的股票进行综合分析
│   │
│   ├── 📁 services/ - 后台服务层
│   │   ├── monitor_service.py          # 实时监测后台服务，daemon线程定时检查价格并触发通知
│   │   ├── notification_service.py     # 通知服务中心，统一管理邮件(SMTP)和Webhook(钉钉/飞书)
│   │   ├── portfolio_manager.py        # 持仓管理器，处理持仓增删改查、批量分析
│   │   ├── portfolio_scheduler.py      # 持仓定时分析调度器，支持多时间点定时分析
│   │   ├── sector_strategy_scheduler.py # 智策定时分析调度器
│   │   └── monitor_scheduler.py        # 实时监测定时调度器，管理交易时段控制
│   │
│   ├── 📁 utils/ - 工具与配置
│   │   ├── config.py                   # 核心配置模块，从.env加载所有环境变量
│   │   ├── model_config.py             # LLM模型配置，定义可用模型列表
│   │   ├── miniqmt_interface.py        # MiniQMT量化交易接口
│   │   ├── smart_monitor_qmt.py        # AI盯盘QMT集成
│   │   ├── smart_monitor_kline.py      # AI盯盘K线图模块
│   │   └── longhubang_scoring.py       # 龙虎榜AI评分系统
│   │
│   └── 📁 pdf/ - PDF报告生成
│       ├── pdf_generator.py            # 核心PDF生成器，基于ReportLab
│       ├── longhubang_pdf.py           # 智瞰龙虎PDF报告
│       ├── sector_strategy_pdf.py      # 智策板块PDF报告
│       └── main_force_pdf_generator.py # 主力选股报告生成器
│
├── 📁 extra/ - 非核心代码（备选/遗留）
│   ├── pdf_generator_fixed.py          # PDF生成器修复版
│   ├── pdf_generator_pandoc.py         # 基于Pandoc的PDF生成器
│   ├── news_announcement_data.py       # 旧版新闻公告数据获取
│   ├── monitor_ui.py                   # 旧版监测UI
│   ├── stm.py                          # AI盯盘独立启动脚本
